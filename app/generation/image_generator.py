...
иллюстрация, визуально передающая смысл трёх отдельных новостей.

Не копируй существующую афишу или рекламный постер целиком. При этом для
film-centric news внутри соответствующей зоны разрешён и предпочтителен
оригинальный самодельный мини-постер, созданный с нуля по правилам раздела 9.
...
сюжетной необходимости. Если выбор между светлым и тёмным оформлением
...
Для film-centric news точное название фильма на оригинальном самодельном
постере или мини-постере является разрешённым и предпочтительным способом
сохранить тематическую связь с новостью.
...
Если новость относится к конкретному фильму, франшизе, релизу, кастингу,
тизеру, трейлеру, продолжению, ремейку, спин-оффу или другой новости,
в которой центральным объектом является именно фильм, ПО УМОЛЧАНИЮ
используй безопасный формат оригинального САМОДЕЛЬНОГО ПОСТЕРА внутри
соответствующей зоны.

Это приоритетный способ визуализации для film-centric news даже в обычном
NORMAL IMAGE режиме, потому что он лучше передаёт тему новости и снижает
риск модерационной блокировки.

Правила такого самодельного постера:

- постер должен быть создан с нуля как новая оригинальная работа;
- он может быть вдохновлён жанром, атмосферой и смыслом новости,
  но не должен быть копией официального постера;
- НЕ копируй композицию, типографику, цветовую схему, ракурс, кадр,
  персонажей, костюмы, лица, позы, фирменные элементы или оформление
  оригинальных рекламных материалов;
- на постере разрешается написать только название фильма или фильмов,
  если это помогает понять новость;
- используй другие цвета, другую композицию и немного другие шрифтовые
  решения без имитации фирменного стиля;
- не используй официальные логотипы студий и франшиз.

Если в одной новости фигурирует НЕСКОЛЬКО фильмов, оформи внутри этой зоны
коллаж из двух или нескольких оригинальных мини-постеров. Каждый такой
мини-постер должен быть самостоятельным и безопасным, а вместе они должны
образовывать цельную редакционную композицию новости.

Если новость требует person-centric подачи и главный смысл несёт конкретный
актёр, режиссёр, музыкант, журналист или другой публичный человек, допускается
сочетать один крупный нейтральный редакционный образ этого человека с одним
или несколькими самодельными постерами/тайтл-карточками фильма на заднем плане
или рядом.
...
профессиональном контексте и, при необходимости, со своим самодельным
постером фильма.
...
Сохранить чёткую тематическую связь с конкретными новостями,
но сделать визуализацию ещё безопаснее и проще для модерации.

Это НЕ режим абстрактных универсальных картинок.
Не заменяй смысл новостей бессодержательными изображениями
кинозала, кинокамеры, случайной толпы или нейтрального офиса,
если по новости можно создать более точную и безопасную визуализацию.

============================================================
1. БАЗОВАЯ СТРАТЕГИЯ FALLBACK
============================================================

Для film-centric news используй ещё более безопасный формат:

- оригинальные самодельные постеры;
- тайтл-карточки с названием фильма;
- коллаж из нескольких самодельных мини-постеров, если в новости
  упоминается несколько фильмов;
- простые тематические фоны, связанные с жанром или атмосферой новости;
- при необходимости один нейтральный центральный человек в безопасном
  профессиональном контексте, но без попытки повторить точную внешность
  персонажа или фирменный образ.

Если название фильма, компании или события важно для понимания новости,
его разрешается показать обычным читаемым текстом.

============================================================
2. ЧТО ДЕЛАТЬ ДЛЯ ФИЛЬМОВ
============================================================

Если новость относится к конкретному фильму или франшизе,
сделай свой оригинальный постер, а не копию официального.

Обязательно соблюдай правила:

- напиши просто название фильма, если это уместно;
- не копируй оригинальный постер полностью или частично;
- не повторяй фирменный стиль один в один;
- не используй изображение узнаваемых героев, характерные костюмы,
  маски, позы, эмблемы и другие защищённые элементы;
- используй другие цвета, другую композицию и другую типографику;
- если новость о нескольких фильмах, сделай коллаж из нескольких
  своих постеров вместо одной общей абстрактной сцены.
...
- профессиональная кино- или сценическая среда;
- безопасный контекст без буквального изображения спорного события.

Для чувствительных, конфликтных или трагических тем не визуализируй
насилие, обвинение, сам спорный эпизод, страдание или сенсационную сцену.
...
8. Не добавляй общий заголовок TOP-3.
9. Не добавляй нумерацию.
10. Не добавляй длинные подписи.

============================================================
5. СТИЛЬ
============================================================

- современная редакционная киноиллюстрация;
- понятная композиция, хорошо читаемая на смартфоне;
- аккуратные оригинальные постеры и коллажи;
- хорошее качество света и материалов;
- единая согласованная цветокоррекция всех трёх зон.
...
Не добавляй текст о модерации, fallback-режиме или внутренних инструкциях.
""".strip()
    ...
    if moderation_safe_editorial_fallback:
        payload: dict[str, object] = {
            ...
            "moderation_safe_editorial_fallback": {
                ...
                "mode": (
                    "safer_title_poster_editorial_v6"
                ),
            },
        }
