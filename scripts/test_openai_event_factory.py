...
def test_factory_arguments() -> None:
    """Проверяет передачу настроек в event runtime."""

    settings = build_settings(
        api_key="sk-local-event-factory-test"
    )
    ...
    print("Event factory arguments: OK")
    print(
    ...
    print(
        "responses_client_type="
        f"{type(runtime.responses_client).__name__}"
    )
    print(
        "evaluator_type="
        f"{type(runtime.evaluator).__name__}"
    )
...
async def test_real_sdk_construction() -> None:
    ...
    settings = build_settings(
        api_key=(
            "sk-local-event-construction-test"
        )
    )
    ...
    assert (
        runtime.evaluator.metadata.model_name
        == "test-model-no-network"
    )
    ...
    print()
    print(
        "Real event AsyncOpenAI construction: OK"
    )
    print(
        "sdk_client_type="
...
async def main() -> int:
    """Запускает проверки event-фабрики."""
    ...
    print(
        "OpenAI event client factory test: OK"
    )
