...
async def prepare_approved_publication(
    ...
    *,
    ...
    """
    Создаёт попытку отправки для существующего одобренного поста.

    Новый publication_batch и generated_post не создаются.
    Повторная отправка блокируется при наличии попытки со статусом
    started, unknown или published.
    """
    ...
    async with pool.acquire() as connection:
        async with connection.transaction():
            ...
            attempt_number = await connection.fetchval(
                """
                SELECT
                    COALESCE(MAX(attempt_number), 0)::integer + 1
                FROM publication_attempts
                WHERE generated_post_id = $1
                """,
                generated_post_id,
