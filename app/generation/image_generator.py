...
Входные данные уже намеренно обезличены.
Для каждой новости передан только safe_visual_brief.

ВАЖНО:

- следуй только safe_visual_brief;
- не пытайся угадывать исходное название фильма;
- не пытайся угадывать франшизу;
- не пытайся угадывать персонажа;
- не пытайся угадывать актёра, режиссёра или другого
  публичного человека;
- не изображай узнаваемую внешность конкретного человека;
- не изображай узнаваемого защищённого персонажа;
- не воспроизводи характерный костюм, маску, эмблему
  или другой фирменный образ персонажа;
- не копируй постеры, кадры или рекламные материалы;
- не используй логотипы;
- не используй названия компаний, фильмов или франшиз;
- вообще не добавляй текст внутрь изображения.

Используй только оригинальные нейтральные
киноиндустриальные сцены:

- кинотеатр;
- зрителей;
- съёмочную площадку;
- кинокамеры и оборудование;
- студийную или производственную среду;
- фестивальный кинозал;
- премьерную атмосферу;
- деловую встречу;
- документы;
- нейтральные архитектурные и городские сцены;
- абстрактные кинематографические силуэты людей.
...
- хорошая глубина кадра;
- качественная единая цветокоррекция;
...
def _build_moderation_safe_news_payload(
    ...
    """
    Формирует обезличенные визуальные brief для fallback.

    Исходные title и summary используются только локально
    для классификации и никогда не попадают в Image API.
    """

    def build_safe_visual_brief(
        ...
        if any(
            ...
            return (
                "Сдержанная сцена памяти, связанная "
                "с киноиндустрией: пустой кинозал, "
                "съёмочная площадка или мягко освещённая "
                "кинематографическая среда без изображения "
        ...
        if any(
            ...
            return (
                "Корпоративная киноиндустриальная сцена: "
                "деловая встреча, документы и современная "
                "студийная среда, передающие смысл "
                "регуляторного или делового решения."
            )

        if any(
            marker in source_text
            for marker in box_office_markers
        ):
            return (
                "Новость об успехе фильма в прокате: "
                "современный кинотеатр, многочисленные "
                "зрители, большой экран и атмосфера "
                "масштабного успешного кинорелиза."
            )

        if any(
            marker in source_text
            for marker in festival_markers
        ):
            return (
                "Кинофестивальная сцена: современный "
                "фестивальный кинозал, публика, экран "
                "и выразительная премьерная атмосфера."
            )
        ...
        if any(
            ...
            return (
                "Новость об анонсе крупного кинопроекта: "
                "студийная презентация, съёмочная площадка, "
                "кинокамера, производственная среда "
                "и нейтральные человеческие силуэты."
            )

        return (
            "Нейтральная современная редакционная сцена "
            "о киноиндустрии: съёмочная площадка, камера, "
            "кинозал или производственная среда."
        )
